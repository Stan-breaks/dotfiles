  ...
  if not pre_0_30_0 then
    if async then
      return mp.command_native_async(
        { name = "subprocess", playback_only = true, args = args, env = "PATH=" .. os.getenv("PATH") }, callback)
    else
...
local function vf_string(filters, full)
  ...
  if full then
    vf = vf ..
        "scale=w=" ..
        effective_w .. ":h=" .. effective_h .. par .. ",pad=w=" .. effective_w ..
        ":h=" .. effective_h .. ":x=-1:y=-1,format=bgra"
  end
...
local function spawn(time)
  ...
  local args = {
    ...
    "--vd-lavc-skiploopfilter=all", "--vd-lavc-software-fallback=1", "--vd-lavc-fast", "--vd-lavc-threads=2", "--hwdec=" ..
  (options.hwdec and "auto" or "no"),
