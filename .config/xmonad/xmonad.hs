...
myKeys =
  ...
  [ ("M-d", spawn myTerminal),
    ("M-f", spawn "librewolf"),
    ("M-a", spawn "pkill -x rofi || rofi -show drun -theme ~/.config/rofi/selector.rasi"),
    ("C-<Print>", spawn "maim -s | xclip -selection clipboard -t image/png"),
    ("M-<Backspace>", spawn  "xmonad --quit"),
    -- Window management
    ("M-q", kill),
    ...
    ("<XF86AudioMute>", spawn "wpctl set-mute @DEFAULT_AUDIO_SOURCE@ toggle")
  ]
    ++
...
myStartupHook :: X ()
myStartupHook = do
  spawnOnce "$HOME/.config/xmonad/startscript.sh"
